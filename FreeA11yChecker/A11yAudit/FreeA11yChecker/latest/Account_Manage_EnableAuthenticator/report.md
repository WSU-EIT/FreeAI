# Page Scan Report

> **URL:** https://localhost:7046/Account/Manage/EnableAuthenticator  
> **Status:** ✅ 200  

---

## Summary

| Field | Value |
|-------|-------|
| URL | https://localhost:7046/Account/Manage/EnableAuthenticator |
| Title | Configure authenticator app |
| Status | ✅ 200 |
| HTML Size | 8.5 KB |
| Screenshots | 17 (1.4 MB) |
| Images | 0 |
| Images Missing Alt | 0 |
| A11y Violations | Warning 7 |
| Critical | 0 |
| Serious | 5 |
| Moderate | 2 |
| Minor | 0 |
| Tools Run | axe, htmlcheck, htmlcs, ibm |

## Screenshots

<table>
<tr>
<td align="center" width="50%">
<a href="01-page-load-00000ms.png">
<img src="01-page-load-00000ms.png" width="400" alt="Page Load +0ms" />
</a>
<br /><strong>1. Page Load +0ms</strong>
<br /><sub>79.2 KB</sub>
</td>
<td align="center" width="50%">
<a href="03-axe-overlay.png">
<img src="03-axe-overlay.png" width="400" alt="axe-overlay" />
</a>
<br /><strong>2. axe-overlay</strong>
<br /><sub>77.3 KB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="04-wave-overlay.png">
<img src="04-wave-overlay.png" width="400" alt="wave-overlay" />
</a>
<br /><strong>3. wave-overlay</strong>
<br /><sub>96.1 KB</sub>
</td>
<td align="center" width="50%">
<a href="05-htmlcs-overlay.png">
<img src="05-htmlcs-overlay.png" width="400" alt="htmlcs-overlay" />
</a>
<br /><strong>4. htmlcs-overlay</strong>
<br /><sub>77.7 KB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="06-ibm-overlay.png">
<img src="06-ibm-overlay.png" width="400" alt="ibm-overlay" />
</a>
<br /><strong>5. ibm-overlay</strong>
<br /><sub>79.0 KB</sub>
</td>
<td align="center" width="50%">
<a href="07-structure-overlay.png">
<img src="07-structure-overlay.png" width="400" alt="structure-overlay" />
</a>
<br /><strong>6. structure-overlay</strong>
<br /><sub>119.3 KB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="07b-wireframe-blueprint.png">
<img src="07b-wireframe-blueprint.png" width="400" alt="wireframe-blueprint" />
</a>
<br /><strong>7. wireframe-blueprint</strong>
<br /><sub>74.3 KB</sub>
</td>
<td align="center" width="50%">
<a href="08-cvd-protanopia.png">
<img src="08-cvd-protanopia.png" width="400" alt="cvd-protanopia" />
</a>
<br /><strong>8. cvd-protanopia</strong>
<br /><sub>84.7 KB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="09-cvd-deuteranopia.png">
<img src="09-cvd-deuteranopia.png" width="400" alt="cvd-deuteranopia" />
</a>
<br /><strong>9. cvd-deuteranopia</strong>
<br /><sub>85.6 KB</sub>
</td>
<td align="center" width="50%">
<a href="10-cvd-tritanopia.png">
<img src="10-cvd-tritanopia.png" width="400" alt="cvd-tritanopia" />
</a>
<br /><strong>10. cvd-tritanopia</strong>
<br /><sub>85.8 KB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="11-cvd-achromatopsia.png">
<img src="11-cvd-achromatopsia.png" width="400" alt="cvd-achromatopsia" />
</a>
<br /><strong>11. cvd-achromatopsia</strong>
<br /><sub>77.7 KB</sub>
</td>
<td align="center" width="50%">
<a href="12-cvd-protanomaly.png">
<img src="12-cvd-protanomaly.png" width="400" alt="cvd-protanomaly" />
</a>
<br /><strong>12. cvd-protanomaly</strong>
<br /><sub>84.3 KB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="13-cvd-deuteranomaly.png">
<img src="13-cvd-deuteranomaly.png" width="400" alt="cvd-deuteranomaly" />
</a>
<br /><strong>13. cvd-deuteranomaly</strong>
<br /><sub>85.8 KB</sub>
</td>
<td align="center" width="50%">
<a href="14-cvd-tritanomaly.png">
<img src="14-cvd-tritanomaly.png" width="400" alt="cvd-tritanomaly" />
</a>
<br /><strong>14. cvd-tritanomaly</strong>
<br /><sub>86.1 KB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="15-screenreader-view.png">
<img src="15-screenreader-view.png" width="400" alt="screenreader-view" />
</a>
<br /><strong>15. screenreader-view</strong>
<br /><sub>88.1 KB</sub>
</td>
<td align="center" width="50%">
<a href="16-reduced-motion.png">
<img src="16-reduced-motion.png" width="400" alt="reduced-motion" />
</a>
<br /><strong>16. reduced-motion</strong>
<br /><sub>81.6 KB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="17-forced-colors.png">
<img src="17-forced-colors.png" width="400" alt="forced-colors" />
</a>
<br /><strong>17. forced-colors</strong>
<br /><sub>67.4 KB</sub>
</td>
<td></td>
</tr>
</table>

## Page Images (0)

*No images found on page.*

## Accessibility

### Cross-Tool Comparison

| Severity | axe | htmlcheck | htmlcs | ibm |
|----------|:---:|:---:|:---:|:---:|
| critical | 0 | 0 | 0 | 0 |
| serious | 0 | 1 | 0 | 4 |
| moderate | 0 | 1 | 0 | 1 |
| minor | 0 | 0 | 0 | 0 |
| **Total** | **0** | **2** | **0** | **5** |

### Violations by Confidence

<details open>
<summary><strong>6 rule(s) violated</strong></summary>

| # | Rule | Severity | Consensus | axe | htmlcheck | htmlcs | ibm | Example |
|--:|------|:--------:|:---------:|:---:|:---:|:---:|:---:|---------|
| 1 | skip-link | moderate | high 1/4 | --- | found | --- | --- |  |
| 2 | aria_form_label_unique | serious | medium 1/4 | --- | --- | --- | found | `<form b-qa4lg9le9h="" method="post" action="Account/Logout">` |
| 3 | div-button | serious | medium 1/4 | --- | found | --- | --- | `<div class="nav-scrollable" onclick="document.querySelect...` |
| 4 | aria_eventhandler_role_valid | serious | medium 1/4 | --- | --- | --- | found | `<div b-qa4lg9le9h="" onclick="document.querySelector('.na...` |
| 5 | input_label_before | serious | medium 1/4 | --- | --- | --- | found | `<input style="" value="" class="form-control valid" name=...` |
| 6 | aria_content_in_landmark | moderate | medium 1/4 | --- | --- | --- | found | `<a b-qa4lg9le9h="" href="" class="navbar-brand">` |

</details>

> **Note:** Automated scanning catches ~30-60% of WCAG issues. Manual keyboard and screen reader testing is still required for full compliance.

## Files

| File | Description |
|------|-------------|
| `01-page-load-00000ms.png` | Page Load +0ms (79.2 KB) |
| `03-axe-overlay.png` | axe-overlay (77.3 KB) |
| `04-wave-overlay.png` | wave-overlay (96.1 KB) |
| `05-htmlcs-overlay.png` | htmlcs-overlay (77.7 KB) |
| `06-ibm-overlay.png` | ibm-overlay (79.0 KB) |
| `07-structure-overlay.png` | structure-overlay (119.3 KB) |
| `07b-wireframe-blueprint.png` | wireframe-blueprint (74.3 KB) |
| `08-cvd-protanopia.png` | cvd-protanopia (84.7 KB) |
| `09-cvd-deuteranopia.png` | cvd-deuteranopia (85.6 KB) |
| `10-cvd-tritanopia.png` | cvd-tritanopia (85.8 KB) |
| `11-cvd-achromatopsia.png` | cvd-achromatopsia (77.7 KB) |
| `12-cvd-protanomaly.png` | cvd-protanomaly (84.3 KB) |
| `13-cvd-deuteranomaly.png` | cvd-deuteranomaly (85.8 KB) |
| `14-cvd-tritanomaly.png` | cvd-tritanomaly (86.1 KB) |
| `15-screenreader-view.png` | screenreader-view (88.1 KB) |
| `16-reduced-motion.png` | reduced-motion (81.6 KB) |
| `17-forced-colors.png` | forced-colors (67.4 KB) |
| `metadata.json` | Machine-readable scan data |
| `a11y-summary.json` | Merged cross-tool accessibility summary |

---

*Generated by FreeA11yChecker Scanner v1.0*
