# Page Scan Report

> **URL:** https://admission.wsu.edu/life/  
> **Status:** ✅ 200  

---

## Summary

| Field | Value |
|-------|-------|
| URL | https://admission.wsu.edu/life/ |
| Title | Life at WSU | Admissions | Washington State University |
| Status | ✅ 200 |
| HTML Size | 119.4 KB |
| Screenshots | 18 (30.2 MB) |
| Images | 6 |
| Images Missing Alt | 0 |
| A11y Violations | Warning 45 |
| Critical | 1 |
| Serious | 23 |
| Moderate | 21 |
| Minor | 0 |
| Tools Run | axe, htmlcheck, htmlcs, ibm |

## Screenshots

<table>
<tr>
<td align="center" width="50%">
<a href="01-page-load-00000ms.png">
<img src="01-page-load-00000ms.png" width="400" alt="Page Load +0ms" />
</a>
<br /><strong>1. Page Load +0ms</strong>
<br /><sub>664.6 KB</sub>
</td>
<td align="center" width="50%">
<a href="02-page-expanded.jpeg">
<img src="02-page-expanded.jpeg" width="400" alt="page-expanded" />
</a>
<br /><strong>2. page-expanded</strong>
<br /><sub>1.1 MB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="03-axe-overlay.png">
<img src="03-axe-overlay.png" width="400" alt="axe-overlay" />
</a>
<br /><strong>3. axe-overlay</strong>
<br /><sub>1.6 MB</sub>
</td>
<td align="center" width="50%">
<a href="04-quickpeek-overlay.png">
<img src="04-quickpeek-overlay.png" width="400" alt="quickpeek-overlay" />
</a>
<br /><strong>4. quickpeek-overlay</strong>
<br /><sub>1.7 MB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="05-htmlcs-overlay.png">
<img src="05-htmlcs-overlay.png" width="400" alt="htmlcs-overlay" />
</a>
<br /><strong>5. htmlcs-overlay</strong>
<br /><sub>1.6 MB</sub>
</td>
<td align="center" width="50%">
<a href="06-ibm-overlay.png">
<img src="06-ibm-overlay.png" width="400" alt="ibm-overlay" />
</a>
<br /><strong>6. ibm-overlay</strong>
<br /><sub>1.6 MB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="07-structure-overlay.png">
<img src="07-structure-overlay.png" width="400" alt="structure-overlay" />
</a>
<br /><strong>7. structure-overlay</strong>
<br /><sub>1.7 MB</sub>
</td>
<td align="center" width="50%">
<a href="07b-wireframe-blueprint.png">
<img src="07b-wireframe-blueprint.png" width="400" alt="wireframe-blueprint" />
</a>
<br /><strong>8. wireframe-blueprint</strong>
<br /><sub>1.2 MB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="08-cvd-protanopia.png">
<img src="08-cvd-protanopia.png" width="400" alt="cvd-protanopia" />
</a>
<br /><strong>9. cvd-protanopia</strong>
<br /><sub>2.0 MB</sub>
</td>
<td align="center" width="50%">
<a href="09-cvd-deuteranopia.png">
<img src="09-cvd-deuteranopia.png" width="400" alt="cvd-deuteranopia" />
</a>
<br /><strong>10. cvd-deuteranopia</strong>
<br /><sub>2.1 MB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="10-cvd-tritanopia.png">
<img src="10-cvd-tritanopia.png" width="400" alt="cvd-tritanopia" />
</a>
<br /><strong>11. cvd-tritanopia</strong>
<br /><sub>2.1 MB</sub>
</td>
<td align="center" width="50%">
<a href="11-cvd-achromatopsia.png">
<img src="11-cvd-achromatopsia.png" width="400" alt="cvd-achromatopsia" />
</a>
<br /><strong>12. cvd-achromatopsia</strong>
<br /><sub>1.4 MB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="12-cvd-protanomaly.png">
<img src="12-cvd-protanomaly.png" width="400" alt="cvd-protanomaly" />
</a>
<br /><strong>13. cvd-protanomaly</strong>
<br /><sub>2.1 MB</sub>
</td>
<td align="center" width="50%">
<a href="13-cvd-deuteranomaly.png">
<img src="13-cvd-deuteranomaly.png" width="400" alt="cvd-deuteranomaly" />
</a>
<br /><strong>14. cvd-deuteranomaly</strong>
<br /><sub>2.1 MB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="14-cvd-tritanomaly.png">
<img src="14-cvd-tritanomaly.png" width="400" alt="cvd-tritanomaly" />
</a>
<br /><strong>15. cvd-tritanomaly</strong>
<br /><sub>2.1 MB</sub>
</td>
<td align="center" width="50%">
<a href="15-screenreader-view.png">
<img src="15-screenreader-view.png" width="400" alt="screenreader-view" />
</a>
<br /><strong>16. screenreader-view</strong>
<br /><sub>703.8 KB</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
<a href="16-reduced-motion.png">
<img src="16-reduced-motion.png" width="400" alt="reduced-motion" />
</a>
<br /><strong>17. reduced-motion</strong>
<br /><sub>2.2 MB</sub>
</td>
<td align="center" width="50%">
<a href="17-forced-colors.png">
<img src="17-forced-colors.png" width="400" alt="forced-colors" />
</a>
<br /><strong>18. forced-colors</strong>
<br /><sub>2.1 MB</sub>
</td>
</tr>
</table>

## Page Images (6)

| # | Source URL | Alt Text |
|--:|-----------|----------|
| 1 | https://wpcdn.web.wsu.edu/wp-admission/uploads/sites/3144/2022/11/The-Spark-a... |  |
| 2 | https://s3.wp.wsu.edu/uploads/sites/3144/2024/01/OC-Butch-NCO-792x733.jpg |  |
| 3 | https://s3.wp.wsu.edu/uploads/sites/3144/2022/11/2022-11-21-14_28_30-Untitled... |  |
| 4 | https://s3.wp.wsu.edu/uploads/sites/3144/2022/11/header_community1.png |  |
| 5 | https://s3.wp.wsu.edu/uploads/sites/3144/2022/11/Shopping-Main1-792x566.jpg |  |
| 6 | https://s3.wp.wsu.edu/uploads/sites/3144/2022/11/northside-exterior-792x528.jpg |  |

## Accessibility

### Cross-Tool Comparison

| Severity | axe | htmlcheck | htmlcs | ibm |
|----------|:---:|:---:|:---:|:---:|
| critical | 1 | 0 | 0 | 0 |
| serious | 3 | 5 | 0 | 15 |
| moderate | 0 | 4 | 0 | 17 |
| minor | 0 | 0 | 0 | 0 |
| **Total** | **4** | **9** | **0** | **32** |

### Violations by Confidence

<details open>
<summary><strong>18 rule(s) violated</strong></summary>

| # | Rule | Severity | Consensus | axe | htmlcheck | htmlcs | ibm | Example |
|--:|------|:--------:|:---------:|:---:|:---:|:---:|:---:|---------|
| 1 | aria-allowed-attr | critical | medium 1/4 | found | --- | --- | --- | `<div id="wsu-navigation-vertical" class="wsu-slide-in-pan...` |
| 2 | color-contrast | serious | medium 1/4 | found | --- | --- | --- | `<strong>Connect</strong>` |
| 3 | image-alt | serious | low 1/4 | --- | found | --- | --- | `<img height="1" width="1" style="display:none" src="https...` |
| 4 | aria_navigation_label_unique | serious | low 1/4 | --- | --- | --- | found | `<nav class="wsu-navigation-audience ">` |
| 5 | table_headers_exists | serious | low 1/4 | --- | --- | --- | found | `<table>` |
| 6 | text_contrast_sufficient | serious | low 1/4 | --- | --- | --- | found | `<span>` |
| 7 | aria_banner_single | serious | low 1/4 | --- | --- | --- | found | `<header class="wsu-header-global  wsu-header-global--styl...` |
| 8 | aria_banner_label_unique | serious | low 1/4 | --- | --- | --- | found | `<header class="wsu-header-global  wsu-header-global--styl...` |
| 9 | aria_contentinfo_label_unique | serious | low 1/4 | --- | --- | --- | found | `<footer class="wsu-footer-site wsu-footer-site--dark">` |
| 10 | link-name | serious | low 1/4 | --- | found | --- | --- | `<a href="https://admission.wsu.edu?s=" class="wsu-button-...` |
| 11 | button-name | serious | low 1/4 | --- | found | --- | --- | `<button class="wsu-search__submit" aria-lable="Submit Sea...` |
| 12 | figure_label_exists | moderate | low 1/4 | --- | --- | --- | found | `<figure class="wp-block-image size-large wsu-image--style...` |
| 13 | aria_landmark_name_unique | moderate | low 1/4 | --- | --- | --- | found | `<header class="wsu-header-global  wsu-header-global--styl...` |
| 14 | table-header | moderate | low 1/4 | --- | found | --- | --- | `<table><tbody><tr><td>48</td><td>U.S. states represented<...` |
| 15 | aria_content_in_landmark | moderate | low 1/4 | --- | --- | --- | found | `<a href="#menu-site-navigation" class="wsu-skip-to-main">` |
| 16 | label | moderate | low 1/4 | --- | found | --- | --- | `<input class="wsu-search__input" type="text" aria-lable="...` |
| 17 | aria_attribute_redundant | moderate | low 1/4 | --- | --- | --- | found | `<select aria-required="true" required="required" name="fo...` |
| 18 | aria_child_valid | moderate | low 1/4 | --- | --- | --- | found | `<ul class="wsu-social-icons">` |

</details>

> **Note:** Automated scanning catches ~30-60% of WCAG issues. Manual keyboard and screen reader testing is still required for full compliance.

## Files

| File | Description |
|------|-------------|
| `01-page-load-00000ms.png` | Page Load +0ms (664.6 KB) |
| `02-page-expanded.jpeg` | page-expanded (1.1 MB) |
| `03-axe-overlay.png` | axe-overlay (1.6 MB) |
| `04-quickpeek-overlay.png` | quickpeek-overlay (1.7 MB) |
| `05-htmlcs-overlay.png` | htmlcs-overlay (1.6 MB) |
| `06-ibm-overlay.png` | ibm-overlay (1.6 MB) |
| `07-structure-overlay.png` | structure-overlay (1.7 MB) |
| `07b-wireframe-blueprint.png` | wireframe-blueprint (1.2 MB) |
| `08-cvd-protanopia.png` | cvd-protanopia (2.0 MB) |
| `09-cvd-deuteranopia.png` | cvd-deuteranopia (2.1 MB) |
| `10-cvd-tritanopia.png` | cvd-tritanopia (2.1 MB) |
| `11-cvd-achromatopsia.png` | cvd-achromatopsia (1.4 MB) |
| `12-cvd-protanomaly.png` | cvd-protanomaly (2.1 MB) |
| `13-cvd-deuteranomaly.png` | cvd-deuteranomaly (2.1 MB) |
| `14-cvd-tritanomaly.png` | cvd-tritanomaly (2.1 MB) |
| `15-screenreader-view.png` | screenreader-view (703.8 KB) |
| `16-reduced-motion.png` | reduced-motion (2.2 MB) |
| `17-forced-colors.png` | forced-colors (2.1 MB) |
| `metadata.json` | Machine-readable scan data |
| `a11y-summary.json` | Merged cross-tool accessibility summary |

---

*Generated by FreeA11yChecker Scanner v1.0*
