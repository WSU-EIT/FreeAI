using FreeA11yChecker.Client.Pages;
using FreeA11yChecker.Components;
...
namespace FreeA11yChecker
{
    public partial class Program
    {
        public static void Main(string[] args)
        {
            ...
            // Log model validation errors for debugging 400 responses.
            builder.Services.Configure<Microsoft.AspNetCore.Mvc.ApiBehaviorOptions>(options => {
                options.InvalidModelStateResponseFactory = context => {
                    var errors = context.ModelState
                        .Where(e => e.Value?.Errors.Count > 0)
                        .Select(e => $"{e.Key}: {String.Join("; ", e.Value!.Errors.Select(x => x.ErrorMessage + (x.Exception != null ? " (" + x.Exception.Message + ")" : "")))}")
                        .ToList();
                    Console.WriteLine("Model validation failed for " + context.HttpContext.Request.Path + ":");
            ...
            try { serverReferences.Add(typeof(FreeA11yChecker.EFModels.EFModels.User).Assembly.Location); } catch { }
            ...
            try { serverReferences.Add(typeof(FreeA11yChecker.Client.BlazorDataModel).Assembly.Location); } catch { }
            ...
            app.MapHub<freeallycheckerHub>("/freeallycheckerHub", signalRConnctionOptions => {
