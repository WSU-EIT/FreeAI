...
The plugin architecture in the FreeBlazorExample is designed to allow for developers that are using
this FreeBlazorExample project as the base for a new solution and they want to be able to add
extendibility to the FreeBlazorExample for their end-users without having to give them access
...
will be included in this base solution as I continue to built out the FreeBlazorExample.
...
to load any necessary assemblies that are not already loaded as part of the FreeBlazorExample
...
In LINQPad you would just add references to the DLL assemblies from the FreeBlazorExample project so
...
and check the FreeBlazorExample\bin\Debug\net10.0 folder you will find the DLL files that you 
need to reference to build against the DataAccess library, the DataObjects, etc.
The main files needed would be: FreeBlazorExample.DataAccess.dll, 
FreeBlazorExample.DataObjects.dll, and FreeBlazorExample.dll. You may also want to reference the
FreeBlazorExample.EFModels.dll file and other libraries if you wish to use methods and
