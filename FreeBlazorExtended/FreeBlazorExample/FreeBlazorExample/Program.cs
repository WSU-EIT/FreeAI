using FreeBlazorExample.Client.Pages;
using FreeBlazorExample.Components;
using FreeBlazorExample.Server.Hubs;
...
namespace FreeBlazorExample
{
    public partial class Program
    {
        public static void Main(string[] args)
        {
            ...
            // Registered server-side too so prerender / InteractiveAuto can resolve them
            builder.Services.AddScoped<FreeBlazorExtended.Foundation.Services.NotificationService>();
            builder.Services.AddScoped<FreeBlazorExtended.DynamicForms.FormService>();
            // Feature 104 — wire EF Core-backed store so prefs persist to SQL on the server.
            // (WASM client falls back to in-memory via the parameterless ctor.)
            ...
            builder.Services.AddScoped<FreeBlazorExtended.AgentMonitoring.IAgentCommandDispatcher, FreeBlazorExtended.AgentMonitoring.SignalRAgentCommandDispatcher>();
            ...
            // External API session store — holds credentials server-side, keyed by HttpOnly cookie
            builder.Services.AddSingleton<ExternalApiSessionStore>();
            ...
            try { serverReferences.Add(typeof(FreeBlazorExample.EFModels.EFModels.User).Assembly.Location); } catch { }
            ...
            try { serverReferences.Add(typeof(FreeBlazorExample.Client.BlazorDataModel).Assembly.Location); } catch { }
            ...
            if (usingStatementsFromConfig != null && usingStatementsFromConfig.Length > 0) {
                foreach (var item in usingStatementsFromConfig) {
                    var normalizedUsingStatement = String.Empty + item;
                    if (normalizedUsingStatement.Equals("using CRM;", StringComparison.OrdinalIgnoreCase)) {
                        normalizedUsingStatement = "using FreeBlazorExample;";
                    }

                    if (!String.IsNullOrWhiteSpace(normalizedUsingStatement) && !usingStatements.Any(x => x.Equals(normalizedUsingStatement, StringComparison.OrdinalIgnoreCase))) {
                        usingStatements.Add(normalizedUsingStatement);
                    }
                }
            }
            ...
            app.MapHub<AppHub>("/appHub", signalRConnctionOptions => {
